...
def main():
    ...
    comp_scores = pd.read_csv(cfg.TAB_DIR / 'ela_composites.csv')
    ...
    for df in [df_base, df_y2, df_y4, df_y6]:
        for col in cfg.COMPOSITE_COLS:
            if col in df.columns:
                df.drop(columns=[col], inplace=True)

    comp_merge = comp_scores[['sub_ID'] + cfg.COMPOSITE_COLS].copy()
    ...
    for label, df in [('baseline', df_base), ('year-2', df_y2),
        ...
        count_nonnan(df, cfg.COMPOSITE_COLS, label)
    ...
    nih_raw = pd.read_csv(cfg.NIH_TOOLBOX_FILE, sep='\t', low_memory=False)
    ...
    nih_cols_needed = ['participant_id', 'session_id'] + [
        c for c in cfg.NIH_COLS if c in nih_raw.columns
    ]
    missing_nih = [c for c in cfg.NIH_COLS if c not in nih_raw.columns]
    if missing_nih:
    ...
    nih_avail_cols = [c for c in cfg.NIH_COLS if c in nih.columns]
    ...
    for _src, _dst in [(cfg.NIH_FLUID_COL, cfg.NIH_FLUID_Y6_COL), (cfg.NIH_CRYST_COL, cfg.NIH_CRYST_Y6_COL)]:
    ...
    cbcl_raw = pd.read_csv(cfg.CBCL_FILE, sep='\t', low_memory=False)
    ...
    cbcl_raw = cbcl_raw.rename(columns={'participant_id': 'sub_ID', **cfg.CBCL_SRC_COLUMNS})

    cbcl_session_map = {'ses-00A': '00A', 'ses-02A': '02A', 'ses-04A': '04A', 'ses-06A': '06A'}

    cbcl_avail_cols = [c for c in cfg.CBCL_MEDIATION_OUTCOMES if c in cbcl_raw.columns]
    missing_cbcl = [c for c in cfg.CBCL_MEDIATION_OUTCOMES if c not in cbcl_raw.columns]
    if missing_cbcl:
