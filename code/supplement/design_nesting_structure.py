...
# ── Family nesting within site ────────────────────────────────────────────────
    ...
    conv_path = cfg.DAT_DIR / 'convergence_diagnostic.csv'
    if conv_path.exists():
    ...
    pd.DataFrame(facts).to_csv(cfg.TAB_DIR / 'supp_design_nesting.csv', index=False)
    ...
    with open(cfg.DAT_DIR / 'supp_design_nesting.txt', 'w') as f:
        f.write('\n'.join(log_lines))
